	...
	components: {
		for name, meta in _addons {
			"\(name)": #OdooAddonBase & {
				...
				if meta.has_tests {
					test: string | *"ODOO_TEST_MAX_FAILED_TESTS=1 uv run --no-sync python $(git rev-parse --show-toplevel)/odoo-bin -d test_odoo_${ENACT_SHARD_INDEX:-1} --http-port=$(( 8069 + ${ENACT_SHARD_INDEX:-1} )) -i \(name) -u \(name) --test-enable --stop-after-init --test-tags $([ -n '{selected_targets}' ] && echo '{selected_targets}' | sed 's|addons/||g; s|^|/|; s| |,/|g' || echo '/\(name)')"
				}
			}
		}
		...
		"base": {
			...
			test: string | *"ODOO_TEST_MAX_FAILED_TESTS=1 uv run --no-sync python $(git rev-parse --show-toplevel)/odoo-bin -d test_odoo_${ENACT_SHARD_INDEX:-1} --http-port=$(( 8069 + ${ENACT_SHARD_INDEX:-1} )) -i base -u base --test-enable --stop-after-init --test-tags /base"
		}
		"web": {
